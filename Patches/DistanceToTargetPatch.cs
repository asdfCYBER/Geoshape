...
namespace Geoshape.Patches
{
    ...
    {
        private static bool Prefix(Vector3 selfPosition, Vector3 targetPosition, ref float __result)
        {
            Vector3 selfNormal = Geometry.GeoscapeToNormal(selfPosition);
            Vector3 targetNormal = Geometry.GeoscapeToNormal(targetPosition);
            __result = Geometry.DistanceBetweenPoints(selfNormal, targetNormal);
